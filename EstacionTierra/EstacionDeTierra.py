                            ...
                            while time.time() - tiempo_inicio < tiempo_max_espera:
                                ...
                                try:
                                    ...
                                    if dist <= 1.0:  # Umbral de llegada en metros
                                        print(f"Llegado al waypoint {idx + 1}")
                                        break
                                except:
...
def video_Websocket_thread():
    ...
    if cap is None:
        cap = cv2.VideoCapture(0)  # (0) es webcam, (1) es camara dron
        if not cap.isOpened():
...
def open_gallery():
    ...
    tk.Button(left_panel, text="Cargar Galería", bg="dark orange", command=load_flight_gallery).pack(fill="x", padx=5)
